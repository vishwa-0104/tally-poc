...
export function MissingEntriesModal({ recordId, companyId, missingRows, onClose }: Props) {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      const result = await syncBankToTally(syncRows, bankLedger, tallyUrl, tallyCompany)
      if (result.success) {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-card rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col">
        ...
        <div className="px-5 py-3 border-b border-border bg-muted/60 flex items-center gap-4 flex-shrink-0">
          <label className="text-xs font-semibold text-muted-foreground whitespace-nowrap">Bank / Cash Ledger *</label>
          <div className="relative">
            <input
              list="missing-bank-ledger-list"
              value={bankLedger}
            ...
            />
            <datalist id="missing-bank-ledger-list">
            ...
            <thead className="sticky top-0 z-10 bg-muted border-b border-border">
              <tr>
                ...
                <th className="px-2 py-2.5 text-right font-semibold text-muted-foreground whitespace-nowrap">
                  Debit<br /><span className="font-normal text-muted-foreground">(Bank Cr)</span>
                </th>
                <th className="px-2 py-2.5 text-right font-semibold text-muted-foreground whitespace-nowrap">
                  Credit<br /><span className="font-normal text-muted-foreground">(Bank Dr)</span>
                </th>
                  ...
                  <td className="px-3 py-1.5">
                    {row.synced ? (
                      ...
                      <>
                        ...
                        />
                        <datalist id={`missing-ledger-${row.id}`}>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-border bg-card flex-shrink-0 gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-3">
            ...
            {!bankLedger.trim() && selectedCount > 0 && (
