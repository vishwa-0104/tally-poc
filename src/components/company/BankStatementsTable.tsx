...
const statusBadgeMap: Record<BankStatementStatus, { variant: 'default' | 'secondary' | 'destructive' | 'outline'; label: string }> = {
  ...
  if (statements.length === 0) {
    return (
      <EmptyState
        ...
        action={<Button onClick={onUpload}>Upload Statement</Button>}
      />
    )
  }
      ...
      {filtered.length === 0 ? (
        ...
        <>
          <div className="overflow-x-auto">
            <Table className="min-w-[720px]" aria-label="Bank statements">
              <TableHeader>
                <TableRow>
                  ...
                  <TableRow key={row.id}>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-semibold">{row.bankName}</p>
                    ...
                    <TableCell><BankStatusBadge status={row.status} /></TableCell>
                    <TableCell className="text-right">
                      {row.status === 'pending' && (
                        <Button size="sm" onClick={() => navigate(`/company/bank/${row.id}`)}>
                          Map & Sync
                        </Button>
                      )}
                      ...
                      {row.status === 'error' && (
                        <Button variant="destructive" size="sm" onClick={() => navigate(`/company/bank/${row.id}`)}>
                          Retry
                        </Button>
                      )}
                      {row.status === 'synced' && (
