...
interface CompanyStore {
  ...
  updateQuota: (companyId: string, data: { parseBillsLimit?: number; parseBlocked?: boolean; parseService?: string; parseModel?: string; subscriptionExpiresAt?: string | null; renew?: boolean }) => Promise<void>
  ...
  addStockGroup: (companyId: string, group: TallyStockGroup) => Promise<void>
  ...
  saveSelectedCreditVoucherType: (companyId: string, creditVoucherType: string) => Promise<void>
...
export const useCompanyStore = create<CompanyStore>((set, get) => ({
  ...
  addStockGroup: async (companyId, group) => {
    const current = get().stockGroups[companyId] ?? []
    await get().saveStockGroupsToDb(companyId, [...current, group])
  },
  ...
  saveSelectedCreditVoucherType: async (companyId, creditVoucherType) => {
    await api.put(`/companies/${companyId}/credit-voucher-type`, { creditVoucherType })
    set((s) => ({
      companies: s.companies.map((c) =>
        c.id === companyId
          ? { ...c, mapping: { ...(c.mapping ?? {}), credit_voucher_type: creditVoucherType } as unknown as Company['mapping'] }
          : c
      ),
    }))
  },
