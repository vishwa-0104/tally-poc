/**
 * Electron main-process port of extension/background.js.
 *
 * Same role, different transport: background.js's chrome.runtime.onMessage
 * listener is replaced by main.cjs's ipcMain.handle('tally-message', ...),
 * which dispatches into the `handlers` map at the bottom of this file by
 * `type` — same type strings, same payload shapes, same response shapes as
 * the extension, so nothing upstream (src/services/tallyService.ts) needs to
 * know which one answered it.
 *
 * The body of this file (everything above "── Dispatch map ──") is a
 * verbatim copy of extension/background.js's helper/handler functions
 * (extracted via `sed -n '183,2234p'`, not retyped, to eliminate
 * transcription risk on ~2000 lines of dense XML-building/regex-parsing
 * logic — including the VOUCHERRETAINKEY===0 ghost-voucher filter in
 * parseVouchers(), which must not drift from the extension's copy). Only
 * Node vs. browser differences would need adapting, and per investigation
 * there are none of consequence: background.js already used nothing but
 * fetch() + string/regex, both native to Node 18+ (what Electron's main
 * process runs) — no chrome.storage/chrome.tabs/chrome.alarms, no
 * DOMParser, no importScripts.
 *
 * If extension/background.js changes, re-run:
 *   sed -n '183,2234p' extension/background.js
 * and diff against this file's body to keep them in sync.
 */
...
// ── Dispatch map ──────────────────────────────────────────────────────────────
// Keyed by the same `type` strings background.js's switch used; each entry's
// payload destructuring and catch-fallback shape mirrors that switch case
// exactly (see extension/background.js lines 25–180).

const handlers = {
  FETCH_LEDGERS: async (payload) => {
    try { return await handleFetchLedgers(payload.tallyUrl, payload.tallyCompany) }
    catch (err) { return { ledgers: [], error: err.message } }
  },

  FETCH_STOCK_ITEMS: async (payload) => {
    try { return await handleFetchStockItems(payload.tallyUrl, payload.tallyCompany) }
    catch (err) { return { stockItems: [], error: err.message } }
  },

  FETCH_STOCK_GROUPS: async (payload) => {
    try { return await handleFetchStockGroups(payload.tallyUrl, payload.tallyCompany) }
    catch (err) { return { stockGroups: [], error: err.message } }
  },

  FETCH_STOCK_UNITS: async (payload) => {
    try { return await handleFetchStockUnits(payload.tallyUrl, payload.tallyCompany) }
    catch (err) { return { stockUnits: [], error: err.message } }
  },

  FETCH_GODOWNS: async (payload) => {
    try { return await handleFetchGodowns(payload.tallyUrl, payload.tallyCompany) }
    catch (err) { return { godowns: [], error: err.message } }
  },

  FETCH_VOUCHER_TYPES: async (payload) => {
    try { return await handleFetchVoucherTypes(payload.tallyUrl, payload.tallyCompany) }
    catch (err) { return { voucherTypes: [], error: err.message } }
  },

  SYNC_TO_TALLY: async (payload) => {
    try { return await handleSyncToTally(payload.xml, payload.tallyUrl) }
    catch (err) { return { success: false, created: 0, altered: 0, errors: 1, message: err.message } }
  },

  CREATE_STOCK_ITEM: async (payload) => {
    try { return await handleCreateStockItem(payload, payload.tallyUrl) }
    catch (err) { return { success: false, created: 0, altered: 0, errors: 1, message: err.message } }
  },

  CREATE_STOCK_GROUP: async (payload) => {
    try { return await handleCreateStockGroup(payload, payload.tallyUrl) }
    catch (err) { return { success: false, created: 0, altered: 0, errors: 1, message: err.message } }
  },

  CREATE_LEDGER: async (payload) => {
    try { return await handleCreateLedger(payload, payload.tallyUrl) }
    catch (err) { return { success: false, created: 0, altered: 0, errors: 1, message: err.message } }
  },

  SYNC_BANK_TO_TALLY: async (payload) => {
    try { return await handleSyncBankToTally(payload.rows, payload.bankLedger, payload.tallyUrl, payload.tallyCompany) }
    catch (err) { return { success: false, created: 0, altered: 0, errors: 1, message: err.message } }
  },

  FETCH_VOUCHERS: async (payload) => {
    try { return await handleFetchVouchers(payload.tallyUrl, payload.tallyCompany, payload.fromDate, payload.toDate, payload.voucherType) }
    catch (err) { return { vouchers: [], error: err.message } }
  },

  FETCH_SALES_PARTY: async (payload) => {
    try { return await handleFetchSalesParty(payload.tallyUrl, payload.tallyCompany, payload.fromDate, payload.toDate) }
    catch (err) { return { parties: [], error: err.message } }
  },

  FETCH_DAYBOOK: async (payload) => {
    try {
      return await handleFetchDaybook(
      ...
      return { vouchers: [], cashFlow: { inflow: 0, outflow: 0 }, bankFlow: { inflow: 0, outflow: 0 }, error: err.message }
    }
  },

  FETCH_SLOW_STOCK: async (payload) => {
    try { return await handleFetchSlowStock(payload.tallyUrl, payload.tallyCompany) }
    catch (err) { return { items: [], error: err.message } }
  },

  FETCH_LEDGER_BALANCES: async (payload) => {
    try { return await handleFetchLedgerBalances(payload.tallyUrl, payload.tallyCompany, payload.asOfDate) }
    catch (err) { return { rawLedgers: [], error: err.message } }
  },

  FETCH_GROUP_BALANCES: async (payload) => {
    try { return await handleFetchGroupBalances(payload.tallyUrl, payload.tallyCompany, payload.asOfDate) }
    catch (err) {
      return {
  ...
  FETCH_STOCK_VALUE: async (payload) => {
    try { return await handleFetchStockValue(payload.tallyUrl, payload.tallyCompany, payload.fromDate, payload.toDate) }
    catch (err) { return { openingStock: 0, closingStock: 0, error: err.message } }
  },

  FETCH_LEDGER_AMOUNTS: async (payload) => {
    try { return await handleFetchLedgerAmounts(payload.tallyUrl, payload.tallyCompany, payload.fromDate, payload.toDate, payload.ledgerNames) }
    catch (err) { return { total: 0, error: err.message } }
  },

  FETCH_DEBTOR_LEDGER_NAMES: async (payload) => {
    try { return await handleFetchDebtorLedgerNames(payload.tallyUrl, payload.tallyCompany) }
    catch (err) { return { names: [], error: err.message } }
  },

  FETCH_LEDGER_CLOSING_BALANCES: async (payload) => {
    try { return await handleFetchLedgerClosingBalances(payload.tallyUrl, payload.tallyCompany, payload.ledgerNames) }
    catch (err) { return { balances: [], error: err.message } }
  },
}

module.exports = { handlers, postToTally, companyVar }
